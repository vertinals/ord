        ...
        match op.action {
            Action::New {
                    ...
                    index_district(context, inscription, op.inscription_id)?
                {
                    ...
                    context.set_inscription_attributes(&inscription_id, &[CollectionKind::BitMap]).map_err(|e|{
                        anyhow!("failed to set inscription attributes! inscription_id: {inscription_id} error: {e}")
                    })?;

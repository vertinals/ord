...
pub struct Context<'a, 'db, 'txn> {
  ...
  // ord tables
  pub(crate) ORD_TX_TO_OPERATIONS: Option<&'a mut Table<'db, 'txn, &'static TxidValue, &'static [u8]>>,
...
impl<'a, 'db, 'txn> OrdReaderWriter for Context<'a, 'db, 'txn> {
  fn save_transaction_operations(
    ...
    save_transaction_operations(*self.ORD_TX_TO_OPERATIONS.as_mut().unwrap(), txid, operations)
  }

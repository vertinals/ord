...
impl SimulateContext {
    pub fn new(
        internal_index: Arc<Index>, simulate_index: IndexTracer) -> crate::Result<Self> {
...
impl SimulateContext {
    ...
    pub fn use_read_simulate_table<K: RedbKey + 'static, V: RedbValue + 'static, T>(&self,
                                                                                    table_def: TableDefinition<K, V>,
                                                                                    f: impl FnOnce(ReadOnlyTable<K, V>) -> crate::Result<T>) -> crate::Result<T> {
        let rtx = self.simulate_index.index.begin_read()?;
        let table = rtx.0.open_table(table_def)?;
        let ret = f(table);
        ret
    }
    pub fn use_write_simulate_table<K: RedbKey + 'static, V: RedbValue + 'static, T>(&self,
                                                                                     table_def: TableDefinition<K, V>,
                                                                                     f: impl FnOnce(Table<K, V>) -> crate::Result<T>) -> crate::Result<T> {
        let rtx = self.simulate_index.index.begin_write()?;
